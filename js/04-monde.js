...
function stunnableCreatures(){
    ...
    /*
    Tout ce qui peut etre sonne, repousse ou leurre. Le
    GARDIEN n'y est pas : un boss qu'on assomme d'un bouton
    n'est plus un boss.
    */
    return [].concat(
        mimics, blobs, gloutons, guimauves, anguilles, lanternes,
        w69Creatures(), theatreCreatures(), foretCreatures()
    );

}


function foretCreatures(){
    return (typeof guepes === "undefined") ? [] : guepes;
}
...
function worldProgress(){
    ...
    if(wd.zone === "foret"){
        ...
        /* elle monte seconde par seconde, pas par paliers */
        const dedans = Math.min(1, levelTimer / FORET_SECS);

        return Math.max(0, Math.min(1, (level - 1 + dedans) / FORET_LEVELS));
...
function updateGuepes(dt){
    ...
    for(const g of guepes){
        ...
        /* sonnee par l'ONDE : elle tombe et ne pique plus */
        if(g.stunned > 0){
        ...
        if(g.phase === "vole"){
            ...
            /* une derive lente autour de sa cible, sans jamais la toucher */
            const cible = lureTarget();

            const dx = cible.x - g.x, dy = cible.y - g.y;
            const d  = Math.hypot(dx, dy) || 1;
            ...
            if(g.t <= 0){
                ...
                sortir de la ligne. Si un LEURRE est pose, c'est
                lui qu'elle vise.
                */
                const cible = lureTarget();
...
function drawGuepes(){
    ...
    for(const g of guepes){
        ...
        /* essoufflee ou sonnee : elle palit, elle ne pique plus */
        if(g.phase === "repos" || g.stunned > 0){
            ctx.globalAlpha = .55;
        }
...
function updateGard(dt){
    ...
    /* il oscille tres lentement, tout au fond */
    gard.x = (a.x0 + a.x1) / 2 + Math.sin(gard.t * .28) * (a.x1 - a.x0) * .08;
    ...
    if(gard.dead > 0){
        ...
        if(gard.dead <= 0){
            ...
            burst((a.x0 + a.x1) / 2, (a.y0 + a.y1) / 2, 70, "#7fd14a");
            burst((a.x0 + a.x1) / 2, (a.y0 + a.y1) / 2, 50, "#ffe89a");
    ...
    /* ---- il attaque : TOUT sort du sol, en bas ---- */
    gard.fire -= dt;
    ...
    if(gard.fire <= 0){
        ...
        if(ph === 0){
            ...
            /*
            LE PEIGNE : un mur de racines sur toute la largeur,
    ...
    for(const r of gardRoots){
        ...
        if(r.phase === "marque"){
            ...
            if(r.t <= 0){
                ...
                burst(r.x, a.y1 - 6 * unit, 12, "#6a4a2a");

                sound(90, .2, "square", .04);

                foretShake = Math.max(foretShake, .22);

            }

        }else if(r.phase === "monte"){

            r.grow = Math.min(1, r.grow + dt / .28);
            ...
            r.grow = Math.max(0, r.grow - dt / .45);

            if(r.t <= 0){ r.mort = true; }

        }

        /* on est touche si on est dans la colonne ET sous sa cime */
        if(r.grow > .35){

            const haut = a.y1 - r.h * r.grow;

            if(Math.abs(player.x - r.x) < r.w / 2 + player.r * .55 &&
               player.y > haut - player.r * .55){
...
function pousseRacine(x, w, h, delai){
    ...
    /*
    Le defaut de la version d'avant : les deux bords etaient
    le miroir l'un de l'autre, ce qui donnait un cone. Ici
    chaque cote a son propre profil, avec ses renflements et
    ses etranglements. C'est l'asymetrie qui fait la racine.
    */
    const courbe = (rnd() - .5) * 1.1;
    const gL     = rnd() * 6.28;
    const gR     = rnd() * 6.28;
    ...
    for(let i = 0; i <= 12; i++){
        ...
        const t    = i / 12;
        /* le sommet garde de la matiere : sinon c'est un brin d'herbe */
        const fine = 1 - Math.pow(t, .85) * .52;

        echine.push({
            t:t,
            dx:courbe * Math.sin(t * 2.4) + Math.sin(t * 8 + gL) * .07,
            wl:fine * (1 + Math.sin(t * 4.1 + gL) * .34 + Math.sin(t * 11 + gL) * .12),
            wr:fine * (1 + Math.sin(t * 3.4 + gR) * .34 + Math.sin(t * 9 + gR) * .12)
        });

    }

    /* les radicelles laterales */
    const bras = [];
    ...
    /* la cassure du bout : trois a cinq echardes inegales */
    /*
    Deux ou trois echardes, l'une nettement plus longue que
    les autres : une cassure reguliere, ca fait un creneau.
    */
    const echardes = [];
    const ne       = 2 + Math.floor(rnd() * 2);
    const roi      = Math.floor(rnd() * ne);

    for(let i = 0; i < ne; i++){
        echardes.push({
            u:(i + .2 + rnd() * .6) / ne,
            len:(i === roi ? .42 + rnd() * .28 : .10 + rnd() * .14),
            pen:(rnd() - .5) * .9
        });
    }
...
function drawGardFront(){
    ...
    for(const r of gardRoots){
        ...
        if(r.phase === "marque"){
            ...
            const k = 1 - r.t / r.marque;

            ctx.save();

            const cg = ctx.createLinearGradient(0, bas, 0, bas - r.h);
            cg.addColorStop(0, "rgba(200,140,60," + (.20 + k * .22).toFixed(3) + ")");
            cg.addColorStop(1, "rgba(200,140,60,0)");

            ctx.fillStyle = cg;
            ctx.fillRect(r.x - r.w / 2, bas - r.h, r.w, r.h);

            ctx.globalAlpha = .35 + Math.sin(r.t * 22) * .25;
            ctx.strokeStyle = "#c98a3a";
            ctx.lineWidth   = 3 * unit;
            ctx.setLineDash([7 * unit, 6 * unit]);

            ctx.beginPath();
            ctx.moveTo(r.x - r.w / 2, bas - 3 * unit);
            ctx.lineTo(r.x + r.w / 2, bas - 3 * unit);
            ctx.moveTo(r.x - r.w / 2, bas - r.h);
            ctx.lineTo(r.x + r.w / 2, bas - r.h);
            ctx.stroke();
...
function dessineRacine(r, bas){
    ...
    /* --- les radicelles, derriere --- */
    ctx.strokeStyle = "#1c1208";
    ctx.lineCap     = "round";

    for(const b of r.bras){

        if(b.t > r.grow){ continue; }

        const p  = pointEchine(r, b.t, bas, h);
        const lo = r.h * b.len * r.grow;

        ctx.lineWidth = r.w * .13;

        ctx.beginPath();
        ctx.moveTo(p.x, p.y);
        ctx.quadraticCurveTo(
            p.x + b.sens * lo * .60, p.y + lo * b.tomb * .30,
            p.x + b.sens * lo * .92, p.y + lo * b.tomb
        );
        ctx.stroke();

    }

    /* --- le corps, asymetrique --- */
    const tete0 = pointEchine(r, 1, bas, h);
    const wl0   = r.w * .5 * ech[n].wl;
    const wr0   = r.w * .5 * ech[n].wr;

    ctx.beginPath();

    for(let i = 0; i <= n; i++){
        const p = pointEchine(r, ech[i].t, bas, h);
        const w = r.w * .5 * ech[i].wl;
        if(i === 0){ ctx.moveTo(p.x - w, p.y); }else{ ctx.lineTo(p.x - w, p.y); }
    }

    /*
    Le sommet n'est pas coupe net : il est CASSE. La dechirure
    fait partie de la silhouette, elle n'est plus une rangee
    de triangles posee par-dessus.
    */
    for(const e of r.echardes){

        const px = tete0.x - wl0 + (wl0 + wr0) * e.u;
        const lo = (wl0 + wr0) * e.len * 1.5;

        ctx.lineTo(px - (wl0 + wr0) * .06, tete0.y + lo * .35);
        ctx.lineTo(px + e.pen * lo * .5,   tete0.y - lo);
        ctx.lineTo(px + (wl0 + wr0) * .07, tete0.y + lo * .2);

    }

    for(let i = n; i >= 0; i--){
        const p = pointEchine(r, ech[i].t, bas, h);
        const w = r.w * .5 * ech[i].wr;
        ctx.lineTo(p.x + w, p.y);
    }

    ctx.closePath();

    /* du bois humide : sombre, a peine chaud, eclaire d'un seul cote */
    const bg = ctx.createLinearGradient(r.x - r.w * .6, 0, r.x + r.w * .6, 0);
    bg.addColorStop(0,   "#120c06");
    bg.addColorStop(.22, "#3e2a15");
    bg.addColorStop(.48, "#54391d");
    bg.addColorStop(.72, "#2b1c0e");
    bg.addColorStop(1,   "#0d0804");

    ctx.fillStyle = bg;
    ctx.fill();

    /* on garde la silhouette pour y enfermer le relief */
    ctx.save();
    ctx.clip();

    /* les cannelures de l'ecorce, dans le sens de la pousse */
    ctx.globalAlpha = .5;
    ctx.strokeStyle = "#0b0703";
    ctx.lineWidth   = Math.max(1, r.w * .05);
    ...
    for(let k = -2; k <= 2; k++){
        ...
        for(let i = 0; i <= n; i++){
            const p = pointEchine(r, ech[i].t, bas, h);
            const o = k * r.w * .16 * (ech[i].wl + ech[i].wr) * .5;
    ...
    /* l'arete claire, sur le bord eclaire */
    ctx.globalAlpha = .28;
    ctx.strokeStyle = "#8a6234";
    ctx.lineWidth   = Math.max(1, r.w * .10);

    ctx.beginPath();

    for(let i = 0; i <= n; i++){
        const p = pointEchine(r, ech[i].t, bas, h);
        const o = -r.w * .5 * ech[i].wl * .55;
        if(i === 0){ ctx.moveTo(p.x + o, p.y); }else{ ctx.lineTo(p.x + o, p.y); }
    }

    ctx.stroke();

    /* la mousse, seulement au pied */
    const mg = ctx.createLinearGradient(0, bas, 0, bas - h * .45);
    mg.addColorStop(0, "rgba(58,92,38,.55)");
    mg.addColorStop(1, "rgba(58,92,38,0)");

    ctx.globalAlpha = 1;
    ctx.fillStyle   = mg;
    ctx.fillRect(r.x - r.w, bas - h * .45, r.w * 2, h * .45);

    ctx.restore();

    /* --- LA CASSURE : du bois brise, pas une pointe --- */
    const tete = pointEchine(r, 1, bas, h);
    const wl   = r.w * .5 * ech[n].wl;
    const wr   = r.w * .5 * ech[n].wr;

    ctx.globalAlpha = 1;

    /* la fibre a vif dans la cassure */
    ctx.globalAlpha = .7;
    ctx.strokeStyle = "#9a7040";
    ctx.lineWidth   = Math.max(1, r.w * .03);
    ctx.lineCap     = "round";

    for(const e of r.echardes){

        const px = tete.x - wl + (wl + wr) * e.u;
        const lo = (wl + wr) * e.len * 1.5;

        ctx.beginPath();
        ctx.moveTo(px, tete.y + lo * .25);
        ctx.lineTo(px + e.pen * lo * .5, tete.y - lo * .88);
        ctx.stroke();
    ...
    /* --- la terre soulevee, et les radicelles au sol --- */
    ctx.globalAlpha = .8 * r.grow;

    ctx.fillStyle = "#241708";
    ctx.beginPath();
    ctx.ellipse(r.x, bas - 2 * unit, r.w * .95, 11 * unit, 0, 0, Math.PI * 2);
    ctx.fill();

    ctx.fillStyle = "#3d2a14";
    ctx.beginPath();
    ctx.ellipse(r.x, bas - 6 * unit, r.w * .68, 8 * unit, 0, 0, Math.PI * 2);
    ctx.fill();

    ctx.strokeStyle = "#1c1208";
    ctx.lineWidth   = Math.max(1, r.w * .07);

    for(let i = 0; i < 5; i++){

        const ang = Math.PI + (i + .5) / 5 * Math.PI;
        const lo  = r.w * (.5 + (i % 2) * .35);

        ctx.beginPath();
        ctx.moveTo(r.x, bas - 5 * unit);
        ctx.quadraticCurveTo(
            r.x + Math.cos(ang) * lo * .6, bas - 5 * unit + Math.sin(ang) * 5 * unit,
            r.x + Math.cos(ang) * lo, bas - 2 * unit
        );
        ctx.stroke();
...
/* la position d'un point de l'echine, a la hauteur atteinte */
function pointEchine(r, t, bas, h){

    const n  = r.echine.length - 1;
    const f  = Math.max(0, Math.min(n, t * n));
    const i  = Math.min(n - 1, Math.floor(f));
    const k  = f - i;

    const dx = r.echine[i].dx * (1 - k) + r.echine[i + 1].dx * k;
...
/* ---------------------------------------------------------
   ...
   Un crane de cerf immense, en bois mort, a moitie noye
   dans la brume. Ses bois se ramifient tout seuls : c'est
   la recursion qui leur donne l'air d'avoir pousse, et pas
   d'avoir ete dessines.
--------------------------------------------------------- */

/* un hasard stable : le meme bois a chaque image */
function gardRnd(n){
...
function boisBranche(c, x, y, ang, len, w, prof, id){

    if(prof <= 0 || len < 3){ return; }

    const cx = x + Math.cos(ang - .28) * len * .55;
    const cy = y + Math.sin(ang - .28) * len * .55;
    ...
    /* la pointe s'arrondit */
    if(prof === 1){
        c.beginPath();
        c.arc(x2, y2, w * .5, 0, Math.PI * 2);
        c.fill();
        return;
    }

    const a = gardRnd(id * 3.1);
    const b = gardRnd(id * 7.7);

    /* la fourche : une branche part sur le cote, l'autre continue */
    boisBranche(c, x2, y2, ang - (.42 + a * .30), len * (.52 + b * .16), w * .58, prof - 1, id * 2 + 1);
    boisBranche(c, x2, y2, ang + (.22 + b * .26), len * (.68 + a * .18), w * .70, prof - 1, id * 2 + 2);

    /* un ergot court, a mi-branche */
    if(prof >= 3){
        const mx = x + Math.cos(ang) * len * .55;
        const my = y + Math.sin(ang) * len * .55;
        boisBranche(c, mx, my, ang - (.9 + a * .5), len * .30, w * .40, prof - 2, id * 2 + 3);
    }

}


function drawGardBack(){

    if(zone !== "foret" || (!gard && foretIntro <= 0)){ return; }
    ...
    if(foretIntro > 0){
        sortie = Math.max(0, Math.min(1, (4.0 - foretIntro) / 2.4));
    }

    if(gard && gard.dead > 0){
        sortie = Math.max(0, gard.dead / 2.4);
    }

    if(sortie <= 0){ return; }

    const cx = gard ? gard.x : (a.x0 + a.x1) / 2;
    const r  = Math.min(W, H) * .30;

    const cy = a.y0 + (a.y1 - a.y0) * .38 + (1 - sortie) * (a.y1 - a.y0) * .55;

    const t   = gard ? gard.t : 0;
    const bal = Math.sin(t * .42) * .026;

    const ph   = GARD_PHASES[gardPhase()];
    const puls = .7 + Math.sin(t * 2.2) * .3;
    ...
    /* --- la brume pale : sans elle, noir sur noir --- */
    const halo = ctx.createRadialGradient(0, -r * .2, r * .1, 0, -r * .2, r * 2.3);
    halo.addColorStop(0,   "rgba(160,200,150,.26)");
    halo.addColorStop(.4,  "rgba(120,165,120,.13)");
    halo.addColorStop(1,   "rgba(110,150,110,0)");

    ctx.fillStyle = halo;
    ctx.fillRect(-r * 2.6, -r * 2.6, r * 5.2, r * 5.2);

    /* --- LES BOIS --- */
    ctx.strokeStyle = "#0a1207";
    ...
    [-1, 1].forEach(sg => {
        ...
        /* la couronne part sur le cote avant de remonter :
           couchee comme ca, elle tient dans le cadre */
        boisBranche(
            ctx,
            sg * r * .34, -r * .48,
            sg > 0 ? -.62 : -(Math.PI - .62),
        ...
        /* une seconde paire, plus courte, vers l'avant */
        boisBranche(
            ctx,
            sg * r * .40, -r * .22,
            sg > 0 ? -.18 : -(Math.PI - .18),
    ...
    /* --- LA MOUSSE PENDANTE --- */
    ctx.strokeStyle = "rgba(120,170,110,.5)";
    ctx.lineWidth   = r * .012;

    for(let i = 0; i < 7; i++){

        const px = (gardRnd(i * 5.3) - .5) * r * 2.1;
        const py = -r * (.75 + gardRnd(i * 9.1) * .55);
        const lo = r * (.16 + gardRnd(i * 2.7) * .28);
        const os = Math.sin(t * .8 + i) * r * .04;

        ctx.beginPath();
        ctx.moveTo(px, py);
        ctx.quadraticCurveTo(px + os, py + lo * .6, px + os * 1.6, py + lo);
        ctx.stroke();
    ...
    kg.addColorStop(0,   "#1a2614");
    kg.addColorStop(.45, "#0e1a0b");
    kg.addColorStop(1,   "#050b04");

    ctx.fillStyle = kg;

    ctx.beginPath();

    /* le front : bas et large, pas une coupole */
    ctx.moveTo(-r * .44, -r * .34);
    ctx.quadraticCurveTo(-r * .46, -r * .66, -r * .17, -r * .70);
    ctx.quadraticCurveTo(0, -r * .72, r * .17, -r * .70);
    ctx.quadraticCurveTo(r * .46, -r * .66, r * .44, -r * .34);

    /* l'arcade, puis le creux de la joue */
    ctx.quadraticCurveTo(r * .47, -r * .16, r * .33, -r * .10);
    ctx.quadraticCurveTo(r * .19, -r * .04, r * .175, r * .10);

    /* le museau : long, droit, il s'affine a peine */
    ctx.quadraticCurveTo(r * .165, r * .48, r * .145, r * .74);
    ctx.quadraticCurveTo(r * .13, r * .90, 0, r * .93);
    ctx.quadraticCurveTo(-r * .13, r * .90, -r * .145, r * .74);
    ctx.quadraticCurveTo(-r * .165, r * .48, -r * .175, r * .10);

    ctx.quadraticCurveTo(-r * .19, -r * .04, -r * .33, -r * .10);
    ctx.quadraticCurveTo(-r * .47, -r * .16, -r * .44, -r * .34);

    ctx.closePath();
    ctx.fill();

    /*
    Le liseret ne fait QUE le haut : eclaire tout autour, le
    crane ressemblait a un ballon detoure.
    */
    ctx.strokeStyle = "rgba(175,215,165,.5)";
    ctx.lineWidth   = r * .016;
    ...
    /* les fissures du bois mort */
    ctx.globalAlpha = sortie * .5;
    ctx.strokeStyle = "#000000";
    ctx.lineWidth   = r * .012;

    for(let i = 0; i < 6; i++){

        const px = (gardRnd(i * 13.7) - .5) * r * .7;

        ctx.beginPath();
        ctx.moveTo(px, -r * .62);
        ctx.quadraticCurveTo(px + (gardRnd(i * 3.3) - .5) * r * .2, -r * .1, px * .5, r * .6);
        ctx.stroke();
    ...
    /* --- LES ORBITES : deux puits noirs --- */
    [-1, 1].forEach(sg => {

        ctx.fillStyle = "#000000";
        ctx.beginPath();
        ctx.moveTo(sg * r * .085, -r * .44);
        ctx.quadraticCurveTo(sg * r * .40, -r * .52, sg * r * .375, -r * .24);
        ctx.quadraticCurveTo(sg * r * .29, -r * .04, sg * r * .10, -r * .16);
        ctx.closePath();
        ctx.fill();
    ...
    [-1, 1].forEach(sg => {
        ...
        const og = ctx.createRadialGradient(ex, ey, 0, ex, ey, r * .22);
        ...
        /*
        Une fente, pas une bille : le reflet blanc rendait
        l'oeil rond et gentil.
        */
        ctx.fillStyle = ph.col;
        ctx.beginPath();
        ctx.ellipse(ex, ey, r * .030, r * .095, sg * .22, 0, Math.PI * 2);
        ctx.fill();

        ctx.fillStyle = "#fff6e0";
        ctx.beginPath();
        ctx.ellipse(ex, ey, r * .012, r * .055, sg * .22, 0, Math.PI * 2);
        ctx.fill();
    ...
    /* --- LA CAVITE NASALE : c'est elle qui dit "crane" --- */
    ctx.fillStyle = "#000000";
    ctx.beginPath();
    ctx.moveTo(0, r * .06);
    ctx.quadraticCurveTo(r * .105, r * .16, r * .058, r * .28);
    ctx.quadraticCurveTo(r * .022, r * .34, 0, r * .30);
    ctx.quadraticCurveTo(-r * .022, r * .34, -r * .058, r * .28);
    ctx.quadraticCurveTo(-r * .105, r * .16, 0, r * .06);
    ctx.closePath();
    ctx.fill();

    /* --- LA GUEULE : elle s'ouvre avec la colere --- */
    const ouvert = r * (.05 + gardPhase() * .085);

    ctx.fillStyle = "#000000";
    ctx.beginPath();
    ctx.ellipse(0, r * .58, r * .135, ouvert, 0, 0, Math.PI * 2);
    ctx.fill();

    /* les dents, deux rangees qui se croisent */
    ctx.fillStyle = "#b9b090";

    for(let i = -2; i <= 2; i++){

        const dx = i * r * .050;
        const lo = ouvert * (i === 0 ? .95 : Math.abs(i) === 1 ? .8 : .55);

        ctx.beginPath();
        ctx.moveTo(dx - r * .022, r * .58 - ouvert);
        ctx.lineTo(dx + r * .022, r * .58 - ouvert);
        ctx.lineTo(dx, r * .58 - ouvert + lo);
        ctx.closePath();
        ctx.fill();
    ...
    /* le souffle qui sort de la gueule quand il enrage */
    if(gardPhase() === 2){

        const sg2 = ctx.createRadialGradient(0, r * .58, 0, 0, r * .58, r * .5);
        sg2.addColorStop(0, hexA(ph.col, .30 * puls));
        sg2.addColorStop(1, hexA(ph.col, 0));

        ctx.fillStyle = sg2;
        ctx.beginPath();
        ctx.arc(0, r * .58, r * .5, 0, Math.PI * 2);
        ctx.fill();
    ...
    /* sa presence assombrit toute l'arene */
    ctx.save();
    ctx.globalAlpha = sortie * .16;
